...
import { predictEligibility } from '../api/client';
import './Card.css';
...
export default function LoanEligibilityCard() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const data = await predictEligibility(form);
      setResult(data);
    } catch (err) {
  ...
  return (
    <section className="card" aria-labelledby="eligibility-heading">
      <h2 id="eligibility-heading" className="card__title">{t('card1.title')}</h2>
      <p className="card__desc">{t('card1.desc')}</p>
      <form className="card__form" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="card__btn" disabled={loading}>
          {loading ? t('card1.checking') : t('card1.submit')}
        </button>
      </form>
      {error && <div className="card__message card__message--error">{error}</div>}
      {result && (
        <div className="card__result">
          <strong>{t('card1.result')}</strong>{' '}
          {result.approved !== undefined
            ? (result.approved ? t('card1.approved') : t('card1.denied'))
            : result.prediction !== undefined
            ? (result.prediction === 1 ? t('card1.approved') : t('card1.denied'))
